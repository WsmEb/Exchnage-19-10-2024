...
<head>
    ...
    <script>
        function info(id, idsolde, valid, type, solde) {
            ...
            document.getElementById(idsolde).value = type == "moi" ? "-" + solde : "+" + solde;
        }
        // setTimeout(function() {
        ...
        <div class="main-panel" id="main-panel">
            ...
            <div class="content">
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header row">
                                    <div class="col-md-4 h5 card-title text-center" style="color: blueviolet; "> Les Historiques des Opérations</div>
                                        ...
                                        <table class="table table-bordered  text-center">
                                            <thead class="">
                                                <tr>
                                                    ...
                                                    <th class="bg-warning">+ {{number_format($total->sommeTotalMoi,2)}}</th>
                                                    <td class="bg-warning">Total toi</td>
                                                    <th class="bg-warning">- {{number_format($total->sommeTotalToi,2)}}</th>
                                                    <td class="bg-warning" colspan="2">Total</td>
                                                    <th class="bg-warning" colspan="2">{{number_format($total->sommeTotalMoi - $total->sommeTotalToi,2)}}</th>
                                                </tr>
                                                <tr>
                                                    <th>Date</th>
                                                    <th>Ville</th>
                                            ...
                                            <tbody id='resultats'>
                                                ...
                                                @foreach ($detail_historiques as $op)
                                                @php
                                                if (session()->has('hover') && in_array($op->id, session('hover')))
                                                {$bg = "bg-info";}
                                                else
                                                {$bg = $op->type_operation === 'moi' ? "bg-success" : "bg-danger";}
                                                @endphp
                                                <tr>
                                                    <td class={{$bg}}>{{ $op->date}}</td>
                                                    <td class={{$bg}}>{{ $op->ville}}</td>
                                                    <td class={{$bg}}>{{ $op->quantity}}</td>
                                                    <td class={{$bg}}>{{ $op->prix}}</td>
                                                    <td class={{$bg}}>{{ $op->comments}}</td>
                                                    <td class={{$bg}}>{{ $op->percentage}}</td>
                                                    <td class={{$bg}}>{{ number_format($op->total,2)}}</td>
                                                    <td class={{$bg}}>
                                                        <a class="btn btn-primary" href="#restoreDetail" data-bs-toggle="modal" onclick="info('id_restore','solde_restore','{{ $op->id}}','{{ $op->type_operation}}','{{ $op->total}}')"><i class="bi bi-arrow-clockwise"></i></a>
                                                        <a class="btn btn-danger border" href="#deleteDetail" data-bs-toggle="modal" onclick="info('id_delete','solde_delete','{{ $op->id}}','{{ $op->type_operation}}','{{ $op->total}}')"><i class="bi bi-trash"></i></a>
                                                    </td>
                                                </tr>
                                                @endforeach
                ...
                <div class="modal fade" id="deleteDetail" tabindex="-1" aria-labelledby="deleteDetailLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form method="POST" action="{{ route('detailhistoriqueoperations.delete') }}">
                                @csrf
                                @method('delete')
                                <div class="modal-header">
                                    <h5 class="modal-title" id="deleteDetailLabel">Supprimer opération</h5>
                                ...
                                <div class="modal-body">
                                    ...
                                    <br>
                                    <p class="text-danger">Tu as sure à supprimer cette opération ?</p>
                                </div>
                ...
                <div class="modal fade" id="restoreDetail" tabindex="-1" aria-labelledby="restoreDetailLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form method="POST" action="{{ route('detailhistoriqueoperations.restore') }}">
                                @csrf
                                <div class="modal-header">
                                    <h5 class="modal-title" id="restoreDetailLabel">Désarchiver opération</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    ...
                                    <p class="text-danger">Tu as sure à désarchiver cette opération ?</p>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
