...
<head>
    ...
    <title>Historiques Converte</title>
    ...
    <script>
        ...
        // }
        function info(id,idsolde, valid,convertedDevise,deviseorigin,solde) {
            document.getElementById(id).value = valid;
            document.getElementById(idsolde).value = convertedDevise == deviseorigin ? "-"+solde : "+"+solde;
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var ConvertedAmount = [];
            var deviseOrigin;
            for(var element of amounts) {

              var ParseEelement = JSON.parse(element.value);
              deviseOrigin = ParseEelement.devise ;
              ParseEelement.devise == ParseEelement.convertedSymbol  ? RecevedAmount.push(ParseEelement.calculatedAmount) : ConvertedAmount.push(ParseEelement.amount);
            }
            var SumRecevedAmount = RecevedAmount.reduce((acc,current) => {
                return acc + current;
            },0);
            var SumConvertedAmount = ConvertedAmount.reduce((acc,current) => {
                return acc + current;
            },0);
            document.getElementById("total_recu").textContent = deviseOrigin ? SumRecevedAmount + ' ' + deviseOrigin : 0 ;
            document.getElementById("total_converted").textContent =  deviseOrigin ? SumConvertedAmount + ' ' + deviseOrigin : 0;
            document.getElementById("total").textContent =  deviseOrigin ? SumRecevedAmount - SumConvertedAmount + ' ' + deviseOrigin : 0;
        ...
        <div class="main-panel" id="main-panel">
            ...
            <div class="content">
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header row">
                                    <div class="col-md-4 h5 card-title text-center" style="color: blueviolet; "> Les Historiques des Convertes</div>
                                ...
                                <div class="card-body" id="table_info">
                                    <div class="table-responsive table-border">
                                        <table class="table table-bordered  text-center">
                                            <thead class="">
                                                <tr>
                                                    <td class="bg-warning" colspan="2">TOTAL RECU</td>
                                                    <th class="bg-warning" id="total_recu"></th>
                                                    <td class="bg-warning">Total CONVERTED</td>
                                                    <th class="bg-warning"  id="total_converted"></th>
                                                    <td class="bg-warning" >TOTAL</td>
                                                    <th class="bg-warning" colspan="2"  id="total"></th>
                                                </tr>
                                                <tr>
                                                    <th>DATE</th>
                                                    <th>CLIENT</th>
                                            ...
                                            <tbody id='resultats'>
                                               ...
                                               { $bg = "bg-info";}
                                                ...
                                                $bg = $historique->devise === $tr->convertedSymbol   ? "bg-success" : "bg-danger";
                                                }
                                                // $type =  $tr->recepteur === $historique->client ? "recepteur" : "expediteur";
                                                @endphp
                                                <tr   >
                                                    <td class={{$bg}}>{{ $tr->date}}</td>
                                                    <td class={{$bg}}>{{ $tr->client_username}}</td>
                                                    <td class={{$bg}}>{{ $tr->devise }}</td>
                                                    <td class={{$bg}}  >{{ $tr->convertedSymbol}}</td>
                                                    <td class={{$bg}} id="amoun" >{{ $tr->amount}} {{$tr->devise}}</td>
                                                    <input type="hidden" id="amount" value='{ "amount": {{$tr->amount}}, "convertedSymbol": "{{$tr->convertedSymbol}}", "devise": "{{$historique->devise}}", "calculatedAmount": {{$tr->amount * ConvertedSymbolBase($tr->devise) / ConvertedSymbolBase($tr->convertedSymbol)}} }'>
                                                    <td class={{$bg}} >{{ $tr->amount  * ConvertedSymbolBase($tr->devise) / ConvertedSymbolBase($tr->convertedSymbol)}} {{$tr->convertedSymbol}}</td>
                                                    <td class={{$bg}}>{{ $tr->commentaire}}</td>
                                                    {{-- <td class={{$bg}}>{{ number_format($tr->solde,2)}}</td> --}}
                                                    <td class={{$bg}} colspan="2">
                                                        <a class="btn btn-primary" href="#restoreDetail" data-bs-toggle="modal" onclick="info(info('id_restore','solde_restore','{{ $tr->id}}','{{ $tr->convertedSymbol}}','{{ $tr->devise}}','{{ $tr->amount}}'))"><i class="bi bi-arrow-clockwise"></i></a>
                                                        <a class="btn btn-danger border" href="#deleteDetail" data-bs-toggle="modal" onclick="info(info('id_delete','solde_delete','{{ $tr->id}}','{{ $tr->convertedSymbol}}','{{ $tr->devise}}','{{ $tr->amount}}'))"><i class="bi bi-trash"></i></a>
                                                    </td>
                                                </tr>
                                                @endforeach
                                                ...
                                                @else
                                                <tr>
                                                    <td colspan="8" class="alert alert-danger text-center fw-bold p-3 text-light">N'est aucun Détail</td>
                                                </tr>
                                                @endif
                                            </tbody>
                ...
                <div class="modal fade" id="deleteDetail" tabindex="-1" aria-labelledby="deleteDetailLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form method="POST" action="{{ route('detailhistoriqueconvertes.delete') }}">
                                @csrf
                                @method('delete')
                                ...
                                <div class="modal-body">
                                    ...
                                    <br>
                                    <p class="text-danger">Tu as sure à supprimer cette converte ?</p>
                                </div>
                ...
                <div class="modal fade" id="restoreDetail" tabindex="-1" aria-labelledby="restoreDetailLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form method="POST" action="{{ route('detailhistoriqueconvertes.restore') }}">
                                @csrf
                                <div class="modal-header">
                                    <h5 class="modal-title" id="restoreDetailLabel">Désarchiver Converte</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    ...
                                    <p class="text-danger">Tu as sure à désarchiver cette Converte ?</p>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
