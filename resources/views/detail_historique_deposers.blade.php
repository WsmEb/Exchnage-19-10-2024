...
<head>
    ...
    <script>
        ...
        function info(id, idsolde, valid, type, solde) {
            ...
            document.getElementById(idsolde).value = type == "DEPOSER" ? "-" + solde : "+" + solde;
        }
        // setTimeout(function() {
        ...
        <div class="main-panel" id="main-panel">
            ...
            <div class="content">
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header row">
                                    <div class="col-md-4 h5 card-title text-center" style="color: blueviolet; "> Les Historiques des Stocks</div>
                                    <div class="col-md-3 p-2">Client : {{$historique->nom_client}}</div>
                                    <div class="col-md-3 p-2">Total terminé : {{number_format($historique->valeur,2)}} {{$historique->devise}}</div>
                                    <div class="col-md-2 p-2"><a class="btn btn-primary" href="#restoreDetail" data-bs-toggle="modal"><i class="bi bi-arrow-clockwise"></i> Désarchiver</a></div>
                                        ...
                                        <table class="table table-bordered  text-center">
                                            <thead class="">
                                                <tr>
                                                    ...
                                                    <th class="bg-warning">+ {{number_format($total->sommeTotalDeposer,2)}}</th>
                                                    <td class="bg-warning">Total toi</td>
                                                    <th class="bg-warning">- {{number_format($total->sommeTotalRetrait,2)}}</th>
                                                    <td class="bg-warning">Total</td>
                                                    <th class="bg-warning">{{number_format($total->sommeTotalDeposer - $total->sommeTotalRetrait,2)}}</th>
                                                </tr>
                                                <tr>
                                                    <th>
                                                        Date
                                            ...
                                            <tbody id='resultats'>
                                                ...
                                                @foreach ($detail_historiques as $st)
                                                @php
                                                $bg = $st->type === "DEPOSER" ? "bg-success" : "bg-danger";
                                                @endphp
                                                <tr>
                                                    <td class={{$bg}}>{{ $st->date_depose}}</td>
                                                    <td class={{$bg}}>{{ $st->type}}</td>
                                                    <td colspan="2" class={{$bg}}>{{ number_format($st->amount,2)}}</td>
                                                    <td class={{$bg}} colspan="2">
                                                        <a class="btn btn-danger border" href="#deleteDetail" data-bs-toggle="modal" onclick="info('id_delete','solde_delete','{{ $st->id}}','{{ $st->type}}','{{ $st->amount}}')"><i class="bi bi-trash"></i></a>
                                                    </td>
                                                </tr>
                                                @endforeach
                ...
                <div class="modal fade" id="deleteDetail" tabindex="-1" aria-labelledby="deleteDetailLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form method="POST" action="{{ route('detailhistoriquedeposers.delete') }}">
                                @csrf
                                @method('delete')
                                <div class="modal-header">
                                    <h5 class="modal-title" id="deleteDetailLabel">Supprimer Stock</h5>
                                ...
                                <div class="modal-body">
                                    ...
                                    <br>
                                    <p class="text-danger">Tu as sure à supprimer cette Stock ?</p>
                                </div>
                ...
                <div class="modal fade" id="restoreDetail" tabindex="-1" aria-labelledby="restoreDetailLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form method="POST" action="{{ route('detailhistoriquedeposers.restore') }}">
                                @csrf
                                <div class="modal-header">
                                    <h5 class="modal-title" id="restoreDetailLabel">Désarchiver stock</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    
                                <input type="hidden" class="form-control" id="idhisto" name="id_hist" value="{{$historique->id}}" />
                                    <div class="form-group">
                                        <label class="form-label">Valeur Terminé :</label>
                                        <input type="text" class="form-control" id="solde_restore" name="solde" pattern="^(\+|-)?\d+(\.\d+)?" title="Entrez un nombre entier ou un nombre flottant (utilisez '.' comme séparateur décimal)"  value="+{{$total->sommeTotalDeposer - $total->sommeTotalRetrait}}" />
                                    </div>
                                    <p class="text-danger">Tu as sure à désarchiver cette stock ?</p>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
