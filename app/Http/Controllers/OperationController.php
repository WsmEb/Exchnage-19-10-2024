    ...
    public function add(Request $request)
    {
        ...
        if ((array_key_exists('operation', $items) && in_array("ajouter", $items["operation"]))) {
            ...
            if (auth()->user()->role == 'comptable') {
                ...
                return  redirect()->route('operation.index', ['client' => $request->client, 'devise' => $request->devise])->with('success', 'Attends L`ACCEPTATION DE ADMIN');
            }else{
        ...
        if ((array_key_exists('operation', $items) && in_array("supprimer", $items["operation"]))) {
            ...
            if (auth()->user()->role == 'comptable') {
                PendingAction::create([
                    ...
                    "model" => "OPERATION",
                    "page" => "operations/".$request->client."/".$request->devise,
                    'details' => json_encode(['id' => $operation->id]),
                ]);
                return  redirect()->route('operation.index', ['client' => $request->client, 'devise' => $request->devise])->with('success', 'Attends L`ACCEPTATION DE ADMIN');
            }else {
                $operation->delete();
